...
static bool has_move(const std::vector<Move> &moves, int fr, int fc, int tr,
                     int tc) {
...
static bool has_promo(const std::vector<Move> &moves, int fr, int fc, int tr,
                      int tc, Piece promo) {
  int from = sq(fr, fc), to = sq(tr, tc);
  return std::any_of(moves.begin(), moves.end(), [&](const Move &m) {
...
static int count_from(const std::vector<Move> &moves, int fr, int fc) {
  ...
  return static_cast<int>(
      std::count_if(moves.begin(), moves.end(),
                    [&](const Move &m) { return m.from_sq == from; }));
}
...
TEST_CASE("Bishop moves on empty board", "[movegen][bishop]") {
  ...
  auto moves = board.generate_legal_moves();
  REQUIRE(count_from(moves, 3, 3) ==
          12); // changed from 13: 14 squares total on diagonals, 1 blocked by
               // king, 1 is the enemy king (now illegal to capture)
}
...
TEST_CASE("Queen moves on empty board", "[movegen][queen]") {
  ...
  auto moves = board.generate_legal_moves();
  REQUIRE(count_from(moves, 3, 3) ==
          26); // changed from 27 (queen can't capture the enemy king on h8)
}
...
TEST_CASE("generate_tactical_moves matches filtered legal moves for Kiwipete",
          "[movegen]") {
  Board board(
      "r3k2r/p1ppqpb1/bn2pnp1/3PN3/1p2P3/2N2Q1p/PPPBBPPP/R3K2R w KQkq - 0 1");
  ...
  for (const auto &m : all_moves) {
    if (board.piece_at_sq(m.to_sq) != Piece::None ||
...
TEST_CASE("generate_tactical_moves includes en passant", "[movegen]") {
  ...
  bool has_ep =
      std::any_of(tactical.begin(), tactical.end(), [](const Move &m) {
        return m.from_sq == sq(4, 5) && m.to_sq == sq(5, 4); // f5xe6 EP
      });
  REQUIRE(has_ep);
}

TEST_CASE("generate_tactical_moves includes promotions from pushes",
          "[movegen]") {
  ...
  for (const auto &m : tactical) {
    if (m.from_sq == sq(6, 0) && m.to_sq == sq(7, 0) &&
        m.promotion != Piece::None) {
      promo_count++;
    }
  }
