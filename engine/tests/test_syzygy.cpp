...
TEST_CASE("Syzygy probe cleanly handles missing files", "[syzygy]") {
    ...
    Board board("8/8/8/8/8/8/3k4/3K4 w - - 0 1");
    SearchResult res = search(board, 3);
    
    // It should evaluate to close to 0 (draw) natively because king vs king is a draw.
    // Positional PSTs might give a slight edge (like +10cp).
    // The key point is it doesn't try to probe and return a mate score.
