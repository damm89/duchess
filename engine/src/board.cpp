...
Move Move::from_uci(const std::string &uci) {
  ...
  if (uci.size() == 5) {
    ...
    switch (uci[4]) {
    case 'q':
      m.promotion = black_promo ? Piece::BlackQueen : Piece::WhiteQueen;
      break;
    case 'r':
      m.promotion = black_promo ? Piece::BlackRook : Piece::WhiteRook;
      break;
    case 'b':
      m.promotion = black_promo ? Piece::BlackBishop : Piece::WhiteBishop;
      break;
    case 'n':
      m.promotion = black_promo ? Piece::BlackKnight : Piece::WhiteKnight;
      break;
    default:
      break;
    }
...
Bitboard Board::white_pieces() const {
  return bb(Piece::WhitePawn) | bb(Piece::WhiteKnight) |
         bb(Piece::WhiteBishop) | bb(Piece::WhiteRook) | bb(Piece::WhiteQueen) |
         bb(Piece::WhiteKing);
}

Bitboard Board::black_pieces() const {
  return bb(Piece::BlackPawn) | bb(Piece::BlackKnight) |
         bb(Piece::BlackBishop) | bb(Piece::BlackRook) | bb(Piece::BlackQueen) |
         bb(Piece::BlackKing);
}
...
int Board::king_square() const {
  Piece king =
      (side_to_move_ == Color::White) ? Piece::WhiteKing : Piece::BlackKing;
  Bitboard k = bb(king);
  return ctzll(k);
}
...
std::string Board::to_fen() const {
  ...
  std::string c;
  if (castling_ & CASTLE_WK)
    c += 'K';
  if (castling_ & CASTLE_WQ)
    c += 'Q';
  if (castling_ & CASTLE_BK)
    c += 'k';
  if (castling_ & CASTLE_BQ)
    c += 'q';
  fen += ' ';
  fen += c.empty() ? "-" : c;
...
bool Board::is_attacked(int square, Color by) const {
  ...
  // Knight
  Piece enemy_knight =
      (by == Color::White) ? Piece::WhiteKnight : Piece::BlackKnight;
  if (KNIGHT_ATTACKS[square] & bb(enemy_knight))
    return true;

  // King
  Piece enemy_king = (by == Color::White) ? Piece::WhiteKing : Piece::BlackKing;
  if (KING_ATTACKS[square] & bb(enemy_king))
    return true;

  // Pawn
  int pawn_color = (by == Color::White)
                       ? 1
                       : 0; // reversed: pawn attacks FROM attacker TO square
  // If white attacks square, a white pawn must be on a square that attacks this
  // square PAWN_ATTACKS[0] = white pawn attack pattern. We check if any white
  // pawn is on a square whose attack pattern includes our target. Equivalently,
  // we use the OPPOSITE color's attack pattern from the target square to find
  // attacking pawns.
  Piece enemy_pawn = (by == Color::White) ? Piece::WhitePawn : Piece::BlackPawn;
  if (PAWN_ATTACKS[pawn_color][square] & bb(enemy_pawn))
    return true;

  // Sliding: bishop/queen on diagonals
  Piece enemy_bishop =
      (by == Color::White) ? Piece::WhiteBishop : Piece::BlackBishop;
  Piece enemy_queen =
      (by == Color::White) ? Piece::WhiteQueen : Piece::BlackQueen;
  Bitboard diag = bishop_attacks(square, occ);
  if (diag & (bb(enemy_bishop) | bb(enemy_queen)))
    return true;
  ...
  if (straight & (bb(enemy_rook) | bb(enemy_queen)))
    return true;
...
void Board::generate_pawn_moves(std::vector<Move> &moves) const {
  ...
  // Actually, simpler: double push is from start rank through one empty to
  // another empty
  Bitboard start_pawns = pawns & (Bitboard(0xFF) << (start_rank * 8));
  Bitboard one_step =
      white ? (start_pawns << 8) & empty : (start_pawns >> 8) & empty;
...
void Board::generate_knight_moves(std::vector<Move> &moves) const {
  Piece knight =
      (side_to_move_ == Color::White) ? Piece::WhiteKnight : Piece::BlackKnight;
...
void Board::generate_bishop_moves(std::vector<Move> &moves) const {
  Piece bishop =
      (side_to_move_ == Color::White) ? Piece::WhiteBishop : Piece::BlackBishop;
...
void Board::generate_rook_moves(std::vector<Move> &moves) const {
  Piece rook =
      (side_to_move_ == Color::White) ? Piece::WhiteRook : Piece::BlackRook;
...
void Board::generate_queen_moves(std::vector<Move> &moves) const {
  Piece queen =
      (side_to_move_ == Color::White) ? Piece::WhiteQueen : Piece::BlackQueen;
...
void Board::generate_king_moves(std::vector<Move> &moves) const {
  ...
  if (ksq == sq(base_row, 4) && !is_attacked(ksq, enemy)) {
    ...
    if ((castling_ & ks_flag) && !test_bit(occ, sq(base_row, 5)) &&
    ...
    if ((castling_ & qs_flag) && !test_bit(occ, sq(base_row, 3)) &&
        !test_bit(occ, sq(base_row, 2)) && !test_bit(occ, sq(base_row, 1)) &&
...
std::vector<Move> Board::generate_legal_moves() const {
  ...
  for (const auto &m : pseudo) {
    ...
    // Never allow capturing the enemy king
    ...
    if (moved == our_king &&
      ...
      if (sq_col(m.to_sq) == 6) {
        // Kingside
        Piece rook = (side_to_move_ == Color::White) ? Piece::WhiteRook
                                                     : Piece::BlackRook;
...
void Board::make_move(const Move &m) {
  ...
  if (!king_moved) {
    ...
    if (w_pt_moved != -1 && b_pt_moved != -1) {
      accumulator_.sub_feature(
          nnue::make_feature(w_ksq, w_pt_moved, m.from_sq),
  ...
  if (moved == Piece::WhiteKing)
    castling_ &= ~(CASTLE_WK | CASTLE_WQ);
  if (moved == Piece::BlackKing)
    castling_ &= ~(CASTLE_BK | CASTLE_BQ);
  if (moved == Piece::WhiteRook && m.from_sq == sq(0, 7))
    castling_ &= ~CASTLE_WK;
  if (moved == Piece::WhiteRook && m.from_sq == sq(0, 0))
    castling_ &= ~CASTLE_WQ;
  if (moved == Piece::BlackRook && m.from_sq == sq(7, 7))
    castling_ &= ~CASTLE_BK;
  if (moved == Piece::BlackRook && m.from_sq == sq(7, 0))
    castling_ &= ~CASTLE_BQ;
  ...
  // Halfmove clock
  if (moved == Piece::WhitePawn || moved == Piece::BlackPawn ||
      captured != Piece::None || is_ep)
    halfmove_clock_ = 0;
  else
    halfmove_clock_++;
