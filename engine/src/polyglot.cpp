...
// Official Polyglot Zobrist random array (781 entries).
// Indices 0-767: pieces (64 * kind + 8 * row + col)
// Indices 768-771: castling rights (WK, WQ, BK, BQ)
// Indices 772-779: en passant file (0-7)
// Index 780: side to move (XOR when white)
static const uint64_t PG_RANDOM[781] = {
...
// Map Duchess Piece enum to Polyglot "kind_of_piece" index.
...
static int piece_to_pg_kind(Piece p) {
    switch (p) {
...
uint64_t polyglot_hash(const Board& board) {
    ...
    // Pieces: index = 64 * kind + 8 * row + col
    for (int s = 0; s < 64; ++s) {
        Piece p = board.piece_at_sq(s);
        if (p == Piece::None) continue;
        int kind = piece_to_pg_kind(p);
        int row = sq_row(s);
        int col = sq_col(s);
        h ^= PG_RANDOM[64 * kind + 8 * row + col];
    }

    // Castling: indices 768-771 (WK, WQ, BK, BQ)
    uint8_t cr = board.castling_rights();
    if (cr & 1) h ^= PG_RANDOM[768];  // WK
    if (cr & 2) h ^= PG_RANDOM[769];  // WQ
    if (cr & 4) h ^= PG_RANDOM[770];  // BK
    if (cr & 8) h ^= PG_RANDOM[771];  // BQ

    // En passant: only hash if a pawn can actually capture
    int ep = board.en_passant_square();
    if (ep >= 0) {
        int ep_col = sq_col(ep);
        int ep_row = sq_row(ep);
        bool can_capture = false;

        // Check if an enemy pawn can capture to the EP square
        if (board.side_to_move() == Color::White) {
            // White pawns on row ep_row-1 at col-1 or col+1
            if (ep_col > 0) {
                Piece lp = board.piece_at(ep_row - 1, ep_col - 1);
                if (lp == Piece::WhitePawn) can_capture = true;
            }
            if (ep_col < 7) {
                Piece rp = board.piece_at(ep_row - 1, ep_col + 1);
                if (rp == Piece::WhitePawn) can_capture = true;
            }
        } else {
            // Black pawns on row ep_row+1 at col-1 or col+1
            if (ep_col > 0) {
                Piece lp = board.piece_at(ep_row + 1, ep_col - 1);
                if (lp == Piece::BlackPawn) can_capture = true;
            }
            if (ep_col < 7) {
                Piece rp = board.piece_at(ep_row + 1, ep_col + 1);
                if (rp == Piece::BlackPawn) can_capture = true;
            }
        }

        if (can_capture) {
            h ^= PG_RANDOM[772 + ep_col];
        }
    }
...
Move polyglot_decode_move(const Board& board, uint16_t raw) {
    ...
    Move m;
    m.from_sq = sq(from_rank, from_file);
    m.to_sq = sq(to_rank, to_file);

    // In Polyglot, castling is encoded as king-to-rook-square.
    // Convert to standard king destination (e1g1, e1c1, e8g8, e8c8).
    Piece mover = board.piece_at_sq(m.from_sq);
    if (mover == Piece::WhiteKing || mover == Piece::BlackKing) {
        if (from_file == 4 && to_file == 7) {
            // King-side castle: to_file becomes g (6)
            m.to_sq = sq(to_rank, 6);
        } else if (from_file == 4 && to_file == 0) {
            // Queen-side castle: to_file becomes c (2)
            m.to_sq = sq(to_rank, 2);
        }
    }

    // Promotion: 0=none, 1=knight, 2=bishop, 3=rook, 4=queen
    if (promo > 0) {
        bool white = (board.side_to_move() == Color::White);
        switch (promo) {
            case 1: m.promotion = white ? Piece::WhiteKnight : Piece::BlackKnight; break;
            case 2: m.promotion = white ? Piece::WhiteBishop : Piece::BlackBishop; break;
            case 3: m.promotion = white ? Piece::WhiteRook   : Piece::BlackRook;   break;
            case 4: m.promotion = white ? Piece::WhiteQueen  : Piece::BlackQueen;  break;
            default: break;
        }
    }
...
// Big-endian read helpers (Polyglot .bin is big-endian)
...
bool OpeningBook::load(const std::string& path) {
    ...
    file.seekg(0);
    size_t count = static_cast<size_t>(size) / 16;
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        BookEntry e;
        e.key = read_u64(p);
        e.raw_move = read_u16(p + 8);
        e.weight = read_u16(p + 10);
        e.learn = read_u32(p + 12);
...
std::vector<BookMove> OpeningBook::probe(const Board& board) const {
    std::vector<BookMove> results;
    if (entries_.empty()) return results;
    ...
    while (it != entries_.end() && it->key == key) {
        BookMove bm;
        bm.move = polyglot_decode_move(board, it->raw_move);
        bm.weight = it->weight;
...
bool OpeningBook::pick_move(const Board& board, Move& out) const {
    auto moves = probe(board);
    if (moves.empty()) return false;

    // Weighted random selection
    uint32_t total = 0;
    for (const auto& bm : moves) total += bm.weight;
    ...
    // Use a random device for non-deterministic selection
